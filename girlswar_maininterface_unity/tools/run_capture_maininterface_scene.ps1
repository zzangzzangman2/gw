...
$Log = Join-Path $Project "logs\unity_maininterface_capture.log"
if (!(Test-Path -LiteralPath $Unity)) { throw "Unity.exe not found: $Unity" }
Start-Process -FilePath $Unity -ArgumentList @(
  "-batchmode",
  "-quit",
  "-projectPath", $Project,
  "-executeMethod", "GirlsWarRestore.MainInterfaceSceneBuilder.CaptureMainInterfaceScene",
  "-logFile", $Log
) -WindowStyle Hidden
"Started Unity background capture. Log: $Log"
