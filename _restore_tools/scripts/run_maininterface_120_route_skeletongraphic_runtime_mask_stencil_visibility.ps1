...
$UnityLog = Join-Path $Reports "unity_120_route_skeletongraphic_runtime_mask_stencil_visibility.log"
$ClickLog = Join-Path $Project "logs\unity_120_click_validation.log"
$ReportScript = Join-Path $Base "_restore_tools\scripts\maininterface120_route_skeletongraphic_runtime_mask_stencil_visibility_report.py"
...
Write-Host "[GirlsWarRestore][120] Tracing route SkeletonGraphic original runtime mask/stencil/attachment visibility"
$unityExit = Invoke-UnityMethod -Method "GirlsWarRestore.MainInterface120RouteSkeletonGraphicRuntimeMaskStencilVisibilityTrace.Trace" -Log $UnityLog
Start-Sleep -Seconds 5

Write-Host "[GirlsWarRestore][120] Running MainInterface click validation"
...
Write-Host "[GirlsWarRestore][120] Writing final report"
python $ReportScript
exit $unityExit
