...
LIVE_JSON = UNITY_DATA / "BATTLE_HUD_CANVAS_SCALER_SPRITE_TEXTURE_LUA_ACTIVE_STATE_CLIP05.json"
LIVE_CSV = UNITY_DATA / "BATTLE_HUD_CANVAS_SCALER_SPRITE_TEXTURE_LUA_ACTIVE_STATE_CLIP05_COMPONENTS.csv"
CAPTURE = UNITY_DIR / "Assets" / "RestoreCaptures" / "battle_hud" / "BattleHudCanvasScalerSpriteTextureLuaStateClip05_1680x720.png"
REFERENCE = REPORT_DIR / "BATTLE_24_PLAY_VIDEO_NORMAL_BATTLE_REFERENCE_486S_SEQUENCE.jpg"
CONTACT = REPORT_DIR / "BATTLE_24_CANVAS_SCALER_SPRITE_TEXTURE_LUA_STATE_CLIP05_CONTACT_SHEET.jpg"
SPRITE_SHEET = REPORT_DIR / "BATTLE_24_VISIBLE_HUD_SPRITE_TEXTURE_SHEET.jpg"
REPORT_JSON = REPORT_DIR / "BATTLE_HUD_CANVAS_SCALER_SPRITE_TEXTURE_LUA_ACTIVE_STATE_CLIP05_RESULT.json"
REPORT_MD = REPORT_DIR / "BATTLE_HUD_CANVAS_SCALER_SPRITE_TEXTURE_LUA_ACTIVE_STATE_CLIP05_RESULT.md"
...
def make_contact_sheet():
    ...
    draw.text((12, 698), "BOTTOM: BATTLE_24 capture. White blocks / missing real scene / blank textures are failure.", fill=(255, 215, 130))
...
def make_sprite_sheet(rows):
    ...
    for target in TARGET_SPRITES:
        ...
        if row is None:
            draw.text((8, 34), "not found in BATTLE_24 CSV", fill=(255, 150, 150))
        else:
...
def main():
    ...
    result = {
        ...
        "final_capture_has_large_white_blocks": True,
        "capture": str(CAPTURE),
        ...
        "failureAxes": {
            ...
            "ui_normalbattle_lua_show_oper_menu_state_missing": True,
            "runtime_left_right_head_skill_buff_assignment_missing": True,
        },
        "summary": summary,
        "luaEvidenceHits": lua_hits[:80],
        "nextBlocker": "BATTLE_25_RESOLVE_SPRITE_TEXTURE_ATLAS_AND_UI_NORMALBATTLE_RUNTIME_BINDING",
    }
    ...
    for row in summary["blankTextureExamples"][:12]:
        md.append(f"- `{row['spriteName']}` `{row['areaRatio']}` `{row['hierarchyPath']}`")
    md += [
        "",
        "## Lua Evidence",
