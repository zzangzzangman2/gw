...
public static class Battle38MatchActorScaleCameraTimingAndBattleSceneContextToClip05Editor
{
    private const string ResultJsonPath = "Assets/RestoreData/battle/BATTLE_38_MATCH_ACTOR_SCALE_CAMERA_TIMING_AND_BATTLE_SCENE_CONTEXT_TO_CLIP05_UNITY.json";
    private const string ActorBoundsCsvPath = "Assets/RestoreData/battle/BATTLE_38_MATCH_ACTOR_SCALE_CAMERA_TIMING_AND_BATTLE_SCENE_CONTEXT_TO_CLIP05_ACTOR_BOUNDS.csv";
    private const string ScenePath = "Assets/Scenes/Battle38MatchActorScaleCameraTimingAndBattleSceneContextToClip05.unity";
    private const string CapturePath = "Assets/RestoreCaptures/battle_actor/Battle38MatchActorScaleCameraTimingAndBattleSceneContextToClip05_1920x1080.png";
    private const string SequenceDir = "Assets/RestoreCaptures/battle_actor/battle38_sequence";
    ...
    [MenuItem("GirlsWar/Battle/BATTLE38 Match Actor Scale Camera Timing And Battle Scene Context To Clip05")]
    public static void Build()
    {
        ...
        var root = new GameObject("BATTLE38_RuntimeActorScaleCameraTimingContextRoot");
        var cameraObject = new GameObject("BATTLE38_ProbeCamera");
        ...
        var actorRows = new List<ActorBoundsRow>();
        ...
        for (int i = 0; i < 6; i++)
        {
            ...
            var sequencePath = ProjectPath(SequenceDir + "/Battle38Runtime_" + i.ToString("00") + "_1920x1080.png");
            Capture(camera, sequencePath);
        }

        Capture(camera, ProjectPath(CapturePath));
        WriteActorCsv(ProjectPath(ActorBoundsCsvPath), actorRows);
        WriteJson(ProjectPath(ResultJsonPath), shaderBundle, actors, actorRows, camera);
        EditorSceneManager.SaveScene(scene, ScenePath);

        foreach (var bundle in opened)
            if (bundle != null) bundle.Unload(false);
        AssetDatabase.Refresh();
        Debug.Log("BATTLE38 actor scale/camera/timing probe complete. actors=" + actors.Count + ", rows=" + actorRows.Count);
    }
    ...
    private static ActorRuntime LoadActor(ActorTarget target, Transform root, List<AssetBundle> opened)
    {
        ...
        runtime.instance.name = "BATTLE38_RuntimeActor_" + target.side + "_" + target.heroDid + "_model_" + target.modelId;
    ...
    private static ActorBoundsRow CreateBoundsRow(ActorRuntime runtime, Camera camera, int frameIndex)
    {
        var row = new ActorBoundsRow();
    ...
    private static string ScreenCenterNorm(Camera camera, Bounds bounds)
    {
        ...
        return (sp.x / 1920f).ToString("0.######") + "/" + (1f - sp.y / 1080f).ToString("0.######");
    }
    ...
    private static void WriteActorCsv(string path, List<ActorBoundsRow> rows)
    {
        var sb = new StringBuilder();
        sb.AppendLine("frame,side,heroDid,modelId,animationName,localScale,rootPosition,worldBounds,screenRect,screenCenterNorm,screenAreaRatio,rendererCount,shaderRebindAppliedCount,meshHash");
        foreach (var row in rows)
        {
            sb.AppendLine(row.frame + "," + Csv(row.side) + "," + Csv(row.heroDid) + "," + Csv(row.modelId) + "," + Csv(row.animationName) + "," + Csv(row.localScale) + "," + Csv(row.rootPosition) + "," + Csv(row.worldBounds) + "," + Csv(row.screenRect) + "," + Csv(row.screenCenterNorm) + "," + row.screenAreaRatio.ToString("0.########") + "," + row.rendererCount + "," + row.shaderRebindAppliedCount + "," + Csv(row.meshHash));
        }
        File.WriteAllText(path, sb.ToString(), new UTF8Encoding(false));
    }

    private static void WriteJson(string path, ShaderBundleInfo shaderBundle, List<ActorRuntime> actors, List<ActorBoundsRow> rows, Camera camera)
    {
        var sb = new StringBuilder();
        sb.AppendLine("{");
        sb.AppendLine("  \"verdict\": \"battle38_actor_scale_camera_timing_context_probe\",");
        ...
        sb.AppendLine("  \"camera\": {\"orthographic\":" + Bool(camera.orthographic) + ",\"orthographicSize\":" + camera.orthographicSize.ToString("0.###") + ",\"position\":\"" + Json(Vec(camera.transform.position)) + "\",\"background\":\"" + Json(camera.backgroundColor.ToString()) + "\"},");
        sb.AppendLine("  \"context\": {\"hasBattleMap\": false, \"hasBattleHud\": false, \"source\": \"BATTLE37_actor_only_runtime_probe\"},");
    ...
    private static string ActorJson(ActorRuntime a, string indent)
    {
        var sb = new StringBuilder();
    ...
    private static int CountMeshHashChanged(List<ActorBoundsRow> rows)
    {
    ...
    private static int Sum(List<ActorRuntime> rows, Func<ActorRuntime, int> selector)
    {
        int count = 0;
    ...
    private sealed class ActorBoundsRow
    {
        public int frame;
        public string side = "";
        public string heroDid = "";
