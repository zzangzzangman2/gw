...
public static class BattleHudRuntimeBindingSpritePptrTraceEditor
{
    ...
    private const string LiveJsonPath = "Assets/RestoreData/battle/BATTLE_HUD_RUNTIME_BINDING_SPRITE_PPTR_TRACE_LIVE.json";
    private const string LiveCsvPath = "Assets/RestoreData/battle/BATTLE_HUD_RUNTIME_BINDING_SPRITE_PPTR_TRACE_LIVE_COMPONENTS.csv";
    private const string CapturePath = "Assets/RestoreCaptures/battle_hud/BattleHudRuntimeBindingSpritePptrTrace_1680x720.png";

    [MenuItem("GirlsWar/Battle/Battle HUD Runtime Binding Sprite PPtr Trace")]
    public static void Build()
    {
        ...
        var rows = new List<BattleHudRuntimeBindingRow>();
        TraceComponents(hudRoot, camera, rows);
        bool captureExists = Capture(camera);
        WriteCsv(ProjectPath(LiveCsvPath), rows);
        WriteJson(ProjectPath(LiveJsonPath), rows, hudRoot != null, hudRoot != null ? hudRoot.transform.childCount : 0, canvasFixCount, captureExists);
        ...
        Debug.Log("BattleHudRuntimeBindingSpritePptrTrace generated. rows=" + rows.Count + ", visibleOriginal=" + Count(rows, r => r.visibleOriginalSpriteCandidate) + ", placeholders=" + Count(rows, r => r.visiblePlaceholderBlock));
    }

    private static GameObject InstantiateLiveHudRoots(List<BattleHudAttachment21> attachments, Dictionary<string, AssetBundle> bundles)
    {
        var root = new GameObject("BattleHudRuntimeBindingTraceLiveHudRoot");
        ...
        foreach (var attachment in attachments)
        {
            ...
            instance.name = "RuntimeTraceHUD_" + attachment.order.ToString("00") + "_" + SafeName(Path.GetFileNameWithoutExtension(attachment.prefabAsset));
            instance.transform.SetParent(root.transform, false);
            var marker = instance.AddComponent<BattleHudRuntimeBindingSourceMarker>();
    ...
    private static void TraceComponents(GameObject hudRoot, Camera camera, List<BattleHudRuntimeBindingRow> rows)
    {
        ...
        foreach (var component in hudRoot.GetComponentsInChildren<Component>(true))
        {
            ...
            var marker = component.GetComponentInParent<BattleHudRuntimeBindingSourceMarker>(true);
            var row = new BattleHudRuntimeBindingRow();
            ...
            row.visiblePlaceholderBlock = row.visibleOnCamera && row.activeInHierarchy && row.enabled && row.role != "Button" && row.areaRatio >= 0.01f && (row.isDefaultWhiteLike || (!row.hasResolvedSprite && (row.role == "Image" || row.role == "RawImage")));
    ...
    private static string Reason(BattleHudRuntimeBindingRow row)
    {
    ...
    private static bool Project(RectTransform rect, Camera camera, BattleHudRuntimeBindingRow row)
    {
    ...
    private static string Zone(BattleHudRuntimeBindingRow row)
    {
    ...
    {
        var go = new GameObject("BattleHudRuntimeBindingTraceCamera");
    ...
    private static void WriteCsv(string path, List<BattleHudRuntimeBindingRow> rows)
    {
    ...
    private static void WriteJson(string path, List<BattleHudRuntimeBindingRow> rows, bool hudRootFound, int liveHudInstantiateCount, int canvasFixCount, bool captureExists)
    {
        Directory.CreateDirectory(Path.GetDirectoryName(path));
    ...
    private static List<BattleHudAttachment21> ReadAttachments(string json)
    {
        var list = new List<BattleHudAttachment21>();
        foreach (string item in ExtractObjectBlocks(ExtractArrayBlock(json, "\"attachments\"")))
        {
            list.Add(new BattleHudAttachment21
            {
    ...
    private static List<BattleHudSpriteBundle21> ReadSpriteBundles(string json)
    {
        var list = new List<BattleHudSpriteBundle21>();
        foreach (string item in ExtractObjectBlocks(ExtractArrayBlock(json, "\"spriteBundles\"")))
        {
            list.Add(new BattleHudSpriteBundle21 { bundle = ReadValue(item, "bundle"), absolutePath = ReadValue(item, "absolutePath") });
        }
        return list;
    }
    ...
    private static int Count(List<BattleHudRuntimeBindingRow> rows, Func<BattleHudRuntimeBindingRow, bool> predicate) { int c = 0; foreach (var r in rows) if (predicate(r)) c++; return c; }
...
public sealed class BattleHudRuntimeBindingSourceMarker : MonoBehaviour
{
    public string role;
...
public sealed class BattleHudAttachment21
{
    public string id;
    public int order;
    public string role;
...
public sealed class BattleHudSpriteBundle21
{
    public string bundle;
    public string absolutePath;
}

public sealed class BattleHudRuntimeBindingRow
{
