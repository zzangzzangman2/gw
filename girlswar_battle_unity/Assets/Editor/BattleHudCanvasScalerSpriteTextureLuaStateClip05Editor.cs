...
public static class BattleHudCanvasScalerSpriteTextureLuaStateClip05Editor
{
    ...
    private const string LiveJsonPath = "Assets/RestoreData/battle/BATTLE_HUD_CANVAS_SCALER_SPRITE_TEXTURE_LUA_ACTIVE_STATE_CLIP05.json";
    private const string LiveCsvPath = "Assets/RestoreData/battle/BATTLE_HUD_CANVAS_SCALER_SPRITE_TEXTURE_LUA_ACTIVE_STATE_CLIP05_COMPONENTS.csv";
    private const string CapturePath = "Assets/RestoreCaptures/battle_hud/BattleHudCanvasScalerSpriteTextureLuaStateClip05_1680x720.png";

    [MenuItem("GirlsWar/Battle/Battle HUD Canvas Scaler Sprite Texture Lua State Clip05")]
    public static void Build()
    {
        ...
        var loadResults = new List<BattleHudBundleLoadResult24>();
        ...
        var rows = new List<BattleHudCanvasScalerRow24>();
        TraceComponents(hudRoot, camera, rows);
        bool captureExists = Capture(camera);
        WriteCsv(ProjectPath(LiveCsvPath), rows);
        WriteJson(ProjectPath(LiveJsonPath), rows, loadResults, canvasFixes, hudRoot != null, hudRoot != null ? hudRoot.transform.childCount : 0, activeStateFixCount, captureExists);
        ...
        Debug.Log("BattleHudCanvasScalerSpriteTextureLuaStateClip05 generated. rows=" + rows.Count + ", deps=" + loadResults.Count + ", visibleOriginal=" + Count(rows, r => r.visibleOriginalSpriteCandidate) + ", placeholders=" + Count(rows, r => r.visiblePlaceholderBlock));
    }

    private static GameObject InstantiateLiveHudRoots(List<BattleHudAttachment24> attachments, Dictionary<string, AssetBundle> bundles)
    {
        var root = new GameObject("BattleHudCanvasScalerSpriteTextureLuaStateClip05Root");
        ...
        foreach (var attachment in attachments)
        {
            ...
            var marker = instance.AddComponent<BattleHudCanvasScalerSourceMarker24>();
    ...
    private static void TraceComponents(GameObject hudRoot, Camera camera, List<BattleHudCanvasScalerRow24> rows)
    {
        ...
        foreach (var component in hudRoot.GetComponentsInChildren<Component>(true))
        {
            ...
            var marker = component.GetComponentInParent<BattleHudCanvasScalerSourceMarker24>(true);
            var row = new BattleHudCanvasScalerRow24();
    ...
    private static string Reason(BattleHudCanvasScalerRow24 row)
    {
    ...
    private static bool Project(RectTransform rect, Camera camera, BattleHudCanvasScalerRow24 row)
    {
    ...
    private static string Zone(BattleHudCanvasScalerRow24 row)
    {
    ...
    private static List<BattleHudCanvasFix24> ApplyCaptureCanvasFix(GameObject hudRoot, Camera camera)
    {
        var fixes = new List<BattleHudCanvasFix24>();
        if (hudRoot == null) return fixes;
        foreach (var canvas in hudRoot.GetComponentsInChildren<Canvas>(true))
        {
            var fix = new BattleHudCanvasFix24();
    ...
    private static void CaptureRectAndCanvas(RectTransform rect, BattleHudCanvasScalerRow24 row)
    {
        if (rect == null) return;
    ...
    private static void FillSpriteInfo(Sprite sprite, BattleHudCanvasScalerRow24 row)
    {
    ...
    {
        var go = new GameObject("BattleHudCanvasScalerSpriteTextureLuaStateClip05Camera");
    ...
    private static void WriteCanvasSummary(StringBuilder sb, List<BattleHudCanvasScalerRow24> rows)
    {
    ...
    private static AssetBundle LoadBundleWithResult(string bundleName, string path, Dictionary<string, AssetBundle> bundles, List<BattleHudBundleLoadResult24> results, string role)
    {
        var result = new BattleHudBundleLoadResult24();
        result.role = role;
    ...
    private static List<BattleHudAttachment24> ReadAttachments(string json)
    {
        var list = new List<BattleHudAttachment24>();
        foreach (string item in ExtractObjectBlocks(ExtractArrayBlock(json, "\"attachments\"")))
        {
            list.Add(new BattleHudAttachment24
            {
    ...
    private static List<BattleHudSpriteBundle24> ReadSpriteBundles(string json)
    {
        var list = new List<BattleHudSpriteBundle24>();
        foreach (string item in ExtractObjectBlocks(ExtractArrayBlock(json, "\"spriteBundles\"")))
        {
            list.Add(new BattleHudSpriteBundle24 { bundle = ReadValue(item, "bundle"), absolutePath = ReadValue(item, "absolutePath") });
        }
        return list;
    }

    private static List<BattleHudSpriteBundle24> ReadDependencyBundles(string json)
    {
        var list = new List<BattleHudSpriteBundle24>();
    ...
    private static int Count(List<BattleHudCanvasScalerRow24> rows, Func<BattleHudCanvasScalerRow24, bool> predicate) { int c = 0; foreach (var r in rows) if (predicate(r)) c++; return c; }
    private static int CountBundle(List<BattleHudBundleLoadResult24> rows, Func<BattleHudBundleLoadResult24, bool> predicate) { int c = 0; foreach (var r in rows) if (predicate(r)) c++; return c; }
...
public sealed class BattleHudCanvasScalerSourceMarker24 : MonoBehaviour
{
    public string role;
...
public sealed class BattleHudAttachment24
{
    public string id;
    public int order;
    public string role;
...
public sealed class BattleHudSpriteBundle24
{
    public string bundle;
    public string absolutePath;
}

public sealed class BattleHudBundleLoadResult24
{
    public string role = "";
    public string bundle = "";
    public string absolutePath = "";
...
public sealed class BattleHudCanvasFix24
{
...
public sealed class BattleHudCanvasScalerRow24
{
